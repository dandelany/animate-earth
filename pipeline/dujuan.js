...
import {
    ...
    // makeVideoCmd, 
    makeButterflowCmd, makeCompressVideoCmd
} from './utils.js';
...
const sessionsToRun = _(sessions)
    ...
    .filter(session => { // filter out sessions < 15 hours long
        ...
        return startDate.isBefore('2015-09-30') && startDate.isAfter('2015-09-23');
...
sessionsToRun.forEach((session, i) => {
    ...
    const sessionDir = `${outPath}/dujuan/${dirName}`;
    ensureDir(sessionDir);

    products.forEach((product, j) => {
        const startDateStr = parseTimeStr(timeStrFromPath(_.first(session.files))).format('YYYY-MM-DD');
        const cropCoord = product.crop;
        const cropDir = `${sessionDir}/${cropCoord}`;
        const imgDir = `${cropDir}/img`;
        // const videoDir = `${cropDir}/video`;
        const videoDir = `${outPath}/dujuan/video`
        const origVideoPath = `${videoDir}/${startDateStr}-lossless.mp4`;
        const origCompressedPath = `${videoDir}/original-${startDateStr}.mp4`;
        // const interpVideoPath = `${videoDir}/interpolated-${origFPS}-${finalFPS}fps-${speed}x-lossless.mp4`;
        const interpCompressedPath = `${videoDir}/interpolated-${startDateStr}.mp4`;
        ...
        execAndLog(makeVideoCmd(imgDir, origFPS, origVideoPath, 60), true, 'original video');
...
export function makeVideoCmd(imgDir, fps, videoPath, realFps) {
    realFps = realFps || fps;
    return `ffmpeg -y -framerate ${fps}\ -pattern_type glob -i '${imgDir}/*.jpg'\
            -c:v libx264 -preset ultrafast -qp 0 -r ${realFps} -pix_fmt yuv420p ${videoPath}`;
}
